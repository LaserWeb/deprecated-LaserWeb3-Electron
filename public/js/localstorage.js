...
function checkSettingsLocal() {
  ...
  for (i = 0; i < localParams.length; i++) {
    ...
    // Laser Spot Size is needed for raster generation, so if unpopulated, I'll rather force a sane default else nothing happens when user clicks Generate gCodeView
    if (!$("#spotSize").val()) {
      $("#spotSize").val("0.1")
    }
  }
