...
function hideCameraOverlay() {
    if (typeof(movieScreen) != 'undefined') {
    ...
    if (window.URL)
    {
        ...
        viewExtents = function(objecttosee) {
            ...
            if (objecttosee) {
                ...
                var fovval = 2.2;
                if (laserxmax > 400) { fovval = 1.5 }
                if (laserxmax > 500) { fovval = 1.4 }
                var fov = fovval * Math.atan(maxlen / (2 * dist)) * (180 / Math.PI);
        ...
        $(window).on('resize', function() {
            ...
            sceneWidth = document.getElementById("renderArea").clientWidth;
            sceneHeight = document.getElementById("renderArea").clientHeight;
            renderer.setSize(sceneWidth, sceneHeight);
